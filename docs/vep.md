...
### 5. Extending the `HyperConverged` API (HCO)

We explored the possibility of adding these configuration profiles directly to the existing `HyperConverged` custom resource (CR) instead of introducing a new API.

**Pros:**
* **Single Entry Point:** Administrators would have one centralized location for all virtualization-related configurations.
* **Simpler RBAC:** No need to define permissions for a new API group.
* **Tighter Integration:** Profile logic could directly access HCO internals without cross-resource coordination.

**Cons:**
* **Separation of Concerns:** The HCO CR is primarily designed for configuring KubeVirt internals (kubevirt, CDI, network-addons operators). Mixing in configuration for external, independent operators (like the Descheduler or Machine Config Operator) risks creating a "God Object" anti-pattern where one controller manages both its own components and unrelated cluster infrastructure.

* **Circular Dependencies:** Some tuning profiles need to adjust KubeVirt settings to align with external tools. For example, the `VirtHigherDensity` profile sets `HyperConverged.spec.higherWorkloadDensity.memoryOvercommitPercentage`. If this logic resided within the HCO controller, it creates a logical loop:
  ```yaml
  # Hypothetical: If profiles lived in HCO
  apiVersion: hco.kubevirt.io/v1beta1
  kind: HyperConverged
  spec:
    # User configures profile here
    advisor:
      profiles:
    ...
    # HCO controller would write here based on advisor.profiles above
    higherWorkloadDensity:
      memoryOvercommitPercentage: 150  # Same resource, circular logic
  ```
  This creates ambiguity: Is `higherWorkloadDensity.memoryOvercommitPercentage` user intent or profile-derived? How does the controller distinguish between manual user changes and profile-driven changes during reconciliation?

* **API Complexity:** The HCO API is already dense with 100+ fields. Adding profile-specific structures (action states, plan items, diffs, optimistic locking, approval workflow) would increase schema bloat and make validation more difficult.

* **Independent Evolution:** Profile logic needs to evolve independently from HCO release cycles. Operator upgrade detection (Section 5) tracks which operator version generated the current plan - embedding this in HCO would conflate HCO versioning with advisor profile versioning.

**Decision:** We opted for a dedicated `VirtPlatformConfig` API to maintain a clean separation between "Platform Core" (HCO manages KubeVirt components) and "Platform Governance/Tuning" (Advisor configures external operators for virtualization optimization). This allows virt-advisor to be an external observer and modifier of HCO, avoiding circular dependencies.

### 6. Decoupled Plan and Approval Resources

We examined a workflow involving two distinct objects: a `VirtPlatformPlan` (generated by the controller) and a separate `VirtPlatformApproval` (created by the user) to strictly separate the proposal from execution.

**Pros:**
* **Explicit Sign-Off:** Provides a very strong audit trail where the "approval" is a distinct, immutable object.
* **Familiarity:** Mimics traditional ITIL change management workflows with separate "change request" and "approval" documents.
* **RBAC Granularity:** Could support separate roles for "plan viewers" vs "approvers" (though this can also be achieved with proper RBAC).

**Cons:**
* **GitOps Friction:** In a declarative GitOps workflow (e.g., ArgoCD, Flux), managing two dependent resources for a single logical change is cumbersome. Consider this scenario:
  ```yaml
  # In Git: clusters/prod/virt-advisor/config.yaml
  apiVersion: advisor.kubevirt.io/v1alpha1
  kind: VirtPlatformConfig
  spec:
    profile: load-aware-rebalancing
  ...
  # Controller creates (NOT in Git):
  # VirtPlatformPlan: load-aware-rebalancing-20250122T153045-001

  # How to approve via Git?
  # Plan name is dynamic (timestamp-based) - unknown at commit time!
  apiVersion: advisor.kubevirt.io/v1alpha1
  kind: VirtPlatformApproval
  metadata:
    name: load-aware-rebalancing-20250122T153045-001-approval  # DYNAMIC!
  spec:
    planRef:
      name: load-aware-rebalancing-20250122T153045-001  # UNKNOWN until runtime!
    approved: true
  ```
  This breaks the declarative GitOps model - resource names must be deterministic at commit time. The user would need out-of-band coordination (check cluster, get plan name, commit approval with dynamic name), defeating the purpose of declarative configuration management.

* **Staleness Risks:** A separate approval object introduces a temporal gap between "Planning" and "Execution." An approval created for "Plan v1" might inadvertently trigger outdated actions if the underlying cluster state changed in the interim. While this can be mitigated with additional validation, it adds complexity.

* **Resource Proliferation:** Each configuration change creates multiple resources. If a user tweaks profile options 5 times during testing, the two-resource pattern creates 10 resources (5 plans + 5 approvals) that require garbage collection policies. The single-resource pattern updates one status field 5 times.

* **Continuous Governance Model Mismatch:** VirtPlatformConfig is a continuous governance resource (drift detection, upgrade detection, input dependency tracking all run continuously), not a one-shot execution. This matches the **Deployment pattern** (single resource, continuous reconciliation) rather than the **Job/CronJob pattern** (template vs execution separation). Forcing a two-resource model onto continuous governance adds unnecessary complexity.

**Decision:** We chose a **Finite State Machine** model on a single object (`action: DryRun` → `Apply`). This aligns with the Operator Lifecycle Manager (OLM) `InstallPlan` pattern and standard GitOps practices, where the "switch" is a field on the primary object:

```yaml
# GitOps-friendly: Approval is declarative spec field
apiVersion: advisor.kubevirt.io/v1alpha1
kind: VirtPlatformConfig
spec:
  profile: load-aware-rebalancing
  action: DryRun  # Review phase
  options: {...}

# Later, commit this change to Git:
spec:
  action: Apply  # Approve - ArgoCD/Flux applies deterministically
```

We mitigate the "staleness" risk via **Optimistic Locking** (`sourceSnapshotHash`) rather than separate resources. The hash-based check ensures that if the target resource changes between plan generation and execution, the apply fails with a clear error requiring plan regeneration. This provides the same safety guarantee as immutable plan resources, without the GitOps friction.

**OLM Precedent:** OLM's `InstallPlan` uses the same pattern - a single resource with `spec.approved` field that users patch to approve. This proves the pattern works well in Kubernetes ecosystems and is familiar to cluster administrators.
